...
object ReliefFFeatureSelectorNB
{
    ...
    def rankFeatures(sc: SparkContext, data: RDD[LabeledPoint], numNeighbors: Int, attributeNumeric: Array[Boolean], discreteClass: Boolean): RDD[(Int, Double)] =
    {
      ...
      if (discreteClass)
      {
        ...
        return selectDiscrete(cart, numberedData, bnTypes, numNeighbors, rangeAttributes, countsClass.toMap, numElems);
      }
      ...
      data.unpersist(false)
      return selectNumeric(cart, numberedData, bnTypes, numNeighbors, rangeAttributes, countsClass.toMap, numElems, rangeClass)
    }
    
    def selectDiscrete(indexPairs: RDD[(Int, Int)], bnData: Array[LabeledPoint], bnTypes: Broadcast[Array[Boolean]], numNeighbors: Int, rangeAttributes:  RDD[(Int, Double)], countsClass: Map[Double, Double], numElems: Double): RDD[(Int, Double)] =
    {
      val dCD=indexPairs
                  ...
                  .flatMap(//Remove comparisons between an instance and itself and compute distances
                  {
                    ...
                    case (x,y) => val feat1=bnData(x).features.toArray
                                  val feat2=bnData(y).features.toArray
                                  var i = 0;
                                  var dist=0.0
...
//.coalesce(144, false)
            .map(//Group by class for each instance so that we can take K neighbors for each class for each instance
                {
                  case (x, nearestNeighborsByClass) => (x, nearestNeighborsByClass.groupBy({case (y, distances) => bnData(y).label}))
                })
            ...
            .map(//Compute multipliers for each addend depending on their class
                {
                  case(x,(y,k)) if (bnData(x).label==bnData(y).label) => (x, y, -1.0/k)
                  case(x,(y,k)) if (bnData(x).label!=bnData(y).label) => (x, y, countsClass.get(bnData(y).label).get/((1.0-countsClass.get(bnData(x).label).get)*k))
                }
                )
            .flatMap(//Separate everything into addends for each attribute, and rearrange so that the attribute index is the key 
                {
                  ...
                  */
                  case(x, y, s) => val feat1=bnData(x).features.toArray
                                  val feat2=bnData(y).features.toArray
                                  var i = 0;
    ...
    def selectNumeric(indexPairs: RDD[(Int, Int)], bnData: Array[LabeledPoint], bnTypes: Broadcast[Array[Boolean]], numNeighbors: Int, rangeAttributes:  RDD[(Int, Double)], countsClass: Map[Double, Double], numElems: Double, rangeClass: Double): RDD[(Int, Double)] =
    {
      ...
      val m_ndc=dCD.map(
                {
                  case (x, (y,k)) => (math.abs(bnData(x).label-bnData(y).label))
